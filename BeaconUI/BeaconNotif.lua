local function init()
    ...
    local has_gethui = type(jjba) == "function"
    local has_gethui2 = false
    pcall(function() if jjba then has_gethui2 = true end end)
    local function uid()
    return tostring(math.floor(tick()*1000)) .. "-" .. http:GenerateGUID(false) end
    local active = {}
...
-- property
    ...
    local function guard(sg)
        ...
        if has_gethui then
            local ok2, gui = pcall(function() return jjba() end)
    ...
    local function remove_node_by_id(id)
        for i = 1, #active do
    ...
    function notifs.CreateNode(opts)
        ...
        local lebron = false
        local nemesis_prime = false
        local function yoshikagekira()
            if lebron or nemesis_prime then return end
            nemesis_prime = true
        ...
        if length and length > 0 then
            spawn(function()
                ...
                while tick() - start < length and not lebron do
                    run.RenderStepped:Wait()
                end
                if not lebron then
                    yoshikagekira()
        ...
        local megatronus = {}
        function megatronus:Close()
            yoshikagekira()
        end
        function megatronus:IsClosed()
            return lebron
        end
        function megatronus:GetFrame()
            return frame
        end
        return megatronus
    end
